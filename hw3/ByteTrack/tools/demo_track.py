    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                timer.toc()
                # print(set(online_ids))
                total_set.update(set(online_ids))
                total_num = len(total_set)
                online_im = plot_tracking(
                    img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / timer.average_time, total_num=total_num
                )
            else:
                timer.toc()
                online_im = img_info['raw_img']
