...
#     return matches, unmatched_a, unmatched_b
def linear_assignment(cost_matrix, thresh):
    strack_num, detection_num = cost_matrix.shape
    unmatched_a = set(np.arange(strack_num))
    unmatched_b = set(np.arange(detection_num))
    x, y = scipy.optimize.linear_sum_assignment(cost_matrix)  
    matches = np.asarray([[x[i], y[i]] for i in range(
        len(x)) if cost_matrix[x[i], y[i]] <= thresh])
    
    if len(matches) > 0:
        unmatched_a = list(
            unmatched_a - set(matches[:, 0]))
        unmatched_b = list(
            unmatched_b - set(matches[:, 1]))
    return matches, unmatched_a, unmatched_b
...
def ious(atlbrs, btlbrs):
    ...
    for i in range(len(atlbrs)):
        for j in range(len(btlbrs)):
            ious[i, j] = calculate_iou(atlbrs[i], btlbrs[j])
    # print("test")
    return ious
...
def calculate_euclidean_distance_cost(center1, center2,scale = 12):
    height = 720/scale
    width = 1280/scale
    
    center1 = (((center1[0] + center1[2]) / 2 / width),
               ((center1[1] + center1[3]) / 2 / height))
    center2 = (((center2[0] + center2[2]) / 2 / width),
               ((center2[1] + center2[3]) / 2 / height))
    # print(center1,center2)
    value = np.linalg.norm(np.array(center1) - np.array(center2))
    # print(value)
    return value


def distance(atlbrs, btlbrs):
    ious = np.zeros((len(atlbrs), len(btlbrs)), dtype=np.float)
    for i in range(len(atlbrs)):
        for j in range(len(btlbrs)):
            ious[i, j] = calculate_euclidean_distance_cost(
                atlbrs[i], btlbrs[j])
    # print("test")
    return ious
